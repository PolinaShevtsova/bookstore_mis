...
function AddBookWindow({isOpen, onClose, obj})
{
   ...
   const [discount, setDiscount] = useState([{id: 1, name: 'Доступна'}, {id: 2, name: 'Не доступна'}]);
   const [genre, setGenre] = useState([]);
   //для ошибок в полях при отправлении формы
   const [error, setError] = useState({title: "", count: "", genre: "", publisher: "",year: "", price: "", description: "", img: "", common: "", authors: "", discount: ""});
   ...
   const fetchDropdownData = async () => {
    ...
    const handleAddBook = async(e) => {
      e.preventDefault();
      let newErrors = {
         title: "", count: "", genre: "", publisher: "",year: "", price: "", 
         description: "", img: "", common: "", authors: "", discount: ""
      };
      let hasError = false;
      ...
      if (!selectedDiscount) {
         newErrors.discount = "Выберите статус книги";
         hasError = true;
      }
      ...
      if (!year) {
         newErrors.year = "Введите ключевые слова";
         hasError = true;
      }
      ...
      formData.append('keywords', year);
      formData.append('status', selectedDiscount.name);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
         if (!isOpen) {
           ...
           setImg('');
           setImgFile(null);
           setError({title: "", count: "", genre: "", publisher: "",year: "",price: "", description: "", img: "", common: "", authors: "", discount: ""});
           return;
         }
         if (obj) {
            setTitle(obj.title || '');
      ...
      <div className={styles.overlay}>
        ...
        <p className={styles.nameWindow}>{obj ? 'Редактирование книги' : 'Добавление книги'}</p>
            <button onClick={() => {onClose(); 
               setError({title: "", count: "", genre: "", publisher: "",year: "",isbn: "",price: "", description: "", img: "", common: "", authors: "", discount: ""});
